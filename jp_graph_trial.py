# funzione chiamata per l'analisi, non viene utilizzata invocandola da terminale
...
def just_processing(a, b, delta, var, use, TIME):
	...
	if use_tof:
		...
		if double_tof:
			...
			tof1_temp = []
			for packet in a:
				tof0_temp = packet['TOF0']
				number_of_samples = len(tof0_temp);
				time_vector = f.building_time(packet, round(float(1000/number_of_samples)));
				partial_tof0 = [list(a) for a in zip(time_vector, tof0_temp)]
				tof0 = tof0 + partial_tof0;
			for packet in b:
				tof1_temp = packet['TOF1']
				number_of_samples = len(tof1_temp);
				time_vector = f.building_time(packet, round(float(1000/number_of_samples)));
				partial_tof1 = [list(b) for b in zip(time_vector, tof1_temp)]
				tof1 = tof1 + partial_tof1;
		...
		interval = int(max_ts_tof-min_ts_tof)
	...
	if check_matching:
		...
		for element in entrate_act:
			...
			check_TOF.append([element + min_ts_tof, 1])
		for element in uscite_act:
			check_out_TOF.append((element + min_ts_tof))
			check_TOF.append([element + min_ts_tof, 3])
		for element in E_inf:
			check_in_INF.append((element))
			check_INF.append([element, 1.5])
		for element in U_inf:
			check_out_INF.append(element)
			check_INF.append([element, 3.5])
		for ing in lista_ingressi:
			GT_IN.append(ing + min_ts_tof)
		for usc in lista_uscite:
			GT_OUT.append(usc + min_ts_tof)
		...
		plt.legend(bbox_to_anchor=(0., 1.02, 1., .102), loc=3, ncol=2, mode="expand", borderaxespad=0.)
		plt.title("Check FP/FN")
		plt.show();
		#plt.plot(*zip(*check_TOF), 'ro', color='blue', label='TOF')
		...
		complete_list = f.cross_check(check_TOF, check_INF, complete_list)
		count_in = 0
		count_out= 0
